...
class rawDataValidation:
    ...
    def __init__(self):
        self.params = yaml.safe_load(open(os.path.join("config","params.yaml")))
    ...
    def valuesFromSchema(self):
        ...
        try:
            training_schema_file = self.params["data_preparation"]["schema_training"]
            ...
            path_for_log = self.params['TrainingLogs']['values_from_schema']

            if not os.path.exists(path_for_log):
                os.makedirs(path_for_log)

            l = open(path_for_log,'a+')
    ...
    def createDirectoryForGoodAndBadRawData(self):
        ...
        """
        try:
            path = self.params['data_preparation']['good_validated_raw_dir']
            if not os.path.isdir(path):
                os.makedirs(path)

            path = self.params['data_preparation']['bad_validated_raw_dir']
            if not os.path.isdir(path):
                os.makedirs(path)

        except OSError as oe:
            path_for_log = self.params['TrainingLogs']['good_bad_data_dir_creation']
    ...
    def moveBadDataFilesToArchievedBad(self):
        ...
        try:
            source = self.params['data_preparation']['bad_validated_raw_dir']
            if os.path.isdir(source):

                path = self.params['data_preparation']['TrainingArchiveBadData']
                if not os.path.isdir(path):
                    os.makedirs(path)
                ...
                path_for_log = self.params['TrainingLogs']['good_bad_data_dir_creation']
                p = open(path_for_log,'a+')
                Logger().log(p,"Bad data file were successfully moved from training raw data folder to the archived bad foler")

                if os.path.isdir(source):
                    shutil.rmtree(source)
                Logger().log(p,"Bad data folder from training raw data folder deleted successfully.")
                p.close()

        except OSError as oe:
            path_for_log = self.params['TrainingLogs']['good_bad_data_dir_creation']
            p = open(path_for_log, 'a+')
            Logger().log(p,"Exception occurred while moving the bad data files from training raw data folder to the archived bad data folder")
            p.close()
            raise oe
    ...
    def validateTrainingDataFileName(self,regex):
        ...
        data_files_path = self.params['data_source']['batch_files']
        raw_data_files = [f for f in os.listdir(data_files_path)]

        path_for_log = self.params['TrainingLogs']['RawDataFileNameValidation']
        ...
        try:
            for file in raw_data_files:
                ...
                if re.match(regex, file):
                    shutil.copy(f"{os.path.join(data_files_path,file)}", "Training_raw_data_validated/GoodData/")
                    Logger().log(f, "Valid file name! File moved to the good data folder")
                    Logger().log(f, "The validation for the data file name passed!")
                    f.close()
                else:
                    shutil.copy(f"{os.path.join(data_files_path,file)}", "Training_raw_data_validated/BadData/")
                    Logger().log(f, "Invalid file name! File moved to the bad data folder")
                    f.close()

        except Exception as e:
            Logger().log(f, f"Exception occurred while validating the file name. Exception: {str(e)}")
            f.close()
            raise e
    ...
    def validateNumberOfColumns(self,numColumns):
        ...
        path_for_log = self.params['TrainingLogs']['numberOfColumnsValidation']
        ...
        try:
            ...
            for file in os.listdir(good_data_path):
                ...
                if csv_file.shape[1] == numColumns:
                    ...
                    shutil.move(os.path.join(good_data_path,file), bad_data_path)
    ...
    def validateMissingValuesInWholeColumn(self):
        ...
        path_for_log = self.params['TrainingLogs']['columnWithAllMissingValuesValidation']
            ...
            for file in os.listdir(good_data_path):
                ...
                for column in columns:
                    ...
                    if noOfMissingValues == csv_file.shape[0]:
                        ...
                        Logger().log(f,"Moving the file " + str(file) + " from good data folder to bad data folder")
                        shutil.move(os.path.join(good_data_path,file), bad_data_path)
                        Logger().log(f,'Moved the file ' + str(file) +  'from good data folder to bad data folder')
                        break
                    else:
                        pass
                Logger().log(f, f"There are no columns with all missing values in the file {file} provided by the client")
            f.close()
...
if __name__ == '__main__':
    ...
    try:
        ...
        obj.validateTrainingDataFileName()

        # validating the number of columns in the raw data file
        obj.validateNumberOfColumns()
